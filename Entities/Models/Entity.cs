    ...
    public class Entity : DynamicObject, IXmlSerializable, IDictionary<string,object>
    {
		...
		public void WriteXml(XmlWriter writer)
		{
			foreach (var key in expando.Keys)
			{
				...
				WriteLinksToXmlElement(key, value, writer);
			}
		}

		private void WriteLinksToXmlElement(string key, object value, XmlWriter writer)
		{
			...
			if (value.GetType() == typeof(List<Link>))
			{
				foreach (var val in value as List<Link>)
				{
					...
					WriteLinksToXmlElement(nameof(val.Href), val.Href, writer);
					WriteLinksToXmlElement(nameof(val.Method), val.Method, writer);
					WriteLinksToXmlElement(nameof(val.Rel), val.Rel, writer);
